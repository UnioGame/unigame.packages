...
`TraceTransport` requires an inner `ITransport` that is also `ISteppedTransport` and whose initial state is exactly `TransportState.Connected/TransportError.None`. It validates all constructor inputs before taking ownership, atomically claims an Open tape as its only writer, owns and disposes the inner transport after successful construction, never owns the tape, and releases the claim in constructor rollback or Dispose. ReplayTransport therefore always exposes the same exact initial `Connected/None` state before its first transcript call, which is the state Session validates in its constructor. Trace Dispose uses `finally` so an inner disposal exception still releases the claim and seals the complete/incomplete tape before rethrowing the original exception. It copies outbound bytes before calling the inner transport so true, false, ordinary throw and transfer-then-throw paths remain observable without borrowing invalid memory.
...
The per-tag invariants are exact:

- `BeginStep`: step is the requested argument; successful return uses flags `1`, thrown call uses flags `2`; channel is `255`; payload length is zero. The current step advances only after successful inner return.
- `TrySend`: step is the most recent successfully begun step, or `ulong.MaxValue` before any successful BeginStep; flags are exactly `0` for false, `1` for true, or `2` for throw; channel is `0` or `1`; payload is the exact complete pre-call packet for every outcome, including false and throw.
- `TryReceive`: step uses the same current-step/sentinel rule; false uses flags `0`, channel `255`, length zero; true uses flags `1`, channel `0` or `1`, and the exact returned packet; throw uses flags `2`, channel `255`, length zero.

No flag combination other than `0`, `1`, or `2` is valid. BeginStep result is represented by successful-return flag `1` even though the source method is void. State and error are sampled after the wrapped call (or after it throws) and become ReplayTransport's exposed state/error after that record is consumed. Replay mismatch does not consume a record and instead sets `Faulted/InvalidPacket` locally.

Complete persisted tapes never contain the throw flag because a wrapped exception marks the in-memory tape incomplete and Save rejects it; the bit remains defined for in-memory validation. The byte capacity applies to the complete record section and every payload length is at most `PacketHeader.Size + ProtocolLimits.MaxWirePayloadBytes`.
